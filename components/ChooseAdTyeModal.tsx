...
import { X, PlayCircle, RadioTower, Wand2 } from 'lucide-react';
...
export const ChooseAdTypeModal: React.FC<ChooseAdTypeModalProps> = ({ onClose, onSelectSkippable, onSelectUnskippable }) => {
  return (
    <div className="fixed inset-0 z-[100] bg-black/70 flex items-center justify-center p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-[var(--background-secondary)] rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-[var(--border-primary)]" onClick={e => e.stopPropagation()}>
        {/* Header */}
