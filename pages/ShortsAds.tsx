...
import { fetchShortsAdCampaigns } from '../services/gemini';
...
export const ShortsAds: React.FC = () => {
  const [campaigns, setCampaigns] = useState<ShortsAdCampaign[]>([]);
  ...
  useEffect(() => {
    const loadCampaigns = async () => {
      setLoading(true);
      const geminiCampaigns = await fetchShortsAdCampaigns();
      ...
      const userShortsAds = userAds.filter(ad => 'impressions' in ad && !('duration' in ad)) as ShortsAdCampaign[];

      setCampaigns([...userShortsAds, ...geminiCampaigns]);
      setLoading(false);
    };
    loadCampaigns();
  }, []);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-[var(--text-primary)] flex items-center gap-3">
            <Smartphone className="w-8 h-8" /> Shorts Ads
          </h1>
          <p className="text-[var(--text-secondary)] mt-1">Manage vertical ad campaigns for the Shorts feed.</p>
        </div>
        {activeTab === 'campaigns' && (
            <button 
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="bg-[var(--background-secondary)] rounded-xl p-4 animate-pulse">
                  <div className="aspect-[9/16] bg-[var(--background-tertiary)] rounded-lg"></div>
                  ...
                  <div className="flex justify-between mt-4 border-t border-[var(--border-primary)] pt-4">
                    <div className="h-8 bg-[var(--background-tertiary)] rounded w-1/3"></div>
                    <div className="h-8 bg-[var(--background-tertiary)] rounded w-1/3"></div>
                  </div>
                </div>
              ))}
