...
const REGIONS = [
    { code: 'US', name: 'United States', currency: '$', price: '9.99' },
    { code: 'IN', name: 'India', currency: '₹', price: '499' },
    { code: 'UK', name: 'United Kingdom', currency: '£', price: '7.99' },
    { code: 'EU', name: 'Europe', currency: '€', price: '8.99' },
    { code: 'JP', name: 'Japan', currency: '¥', price: '1100' },
];
      ...
      if (allProfileDetailsJSON) {
        try {
          ...
          if (userDetails?.country) {
            ...
            if (country === 'India') setSelectedRegionCode('IN');
            else if (country === 'United Kingdom') setSelectedRegionCode('UK');
            else if (['Germany', 'France', 'Italy', 'Spain'].includes(country)) setSelectedRegionCode('EU');
            else if (country === 'Japan') setSelectedRegionCode('JP');
            else setSelectedRegionCode('US');
          }
        } catch (e) {
  ...
  return (
    <div className="w-full h-full bg-[var(--background-primary)] text-[var(--text-primary)] overflow-y-auto relative">
      ...
      <div className="max-w-4xl mx-auto py-12 px-6 text-center">
        ...
        <p className="text-xl text-[var(--text-secondary)] max-w-2xl mx-auto mb-8 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left mb-12 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-300">
          {benefits.map(benefit => (
            <div key={benefit.title} className="bg-[var(--background-secondary)] p-6 rounded-2xl border border-[var(--border-primary)] hover:-translate-y-1 transition-transform shadow-sm">
        ...
        {isPremium ? (
          ...
          <div className="bg-[var(--background-secondary)] p-8 rounded-2xl border border-[var(--border-primary)] shadow-xl animate-in fade-in slide-in-from-bottom-4 duration-500 delay-400">
            ...
            <p className="text-[var(--text-secondary)] mt-2 mb-6">Start your enhanced experience today.</p>
            
            <div className="flex flex-col items-center">
               <div className="text-4xl font-bold mb-2 flex items-baseline">
                   <span className="text-2xl mr-1">{currentRegion.currency}</span>
                   {currentRegion.price}
